...
\begin{itemize}
  \item[--] the simple empirical model of \citet{makkink57},
  \item[--] the half-empirical model of Penman \& Monteith \citep{monteith65},
  \item[--] the half-empirical model of Penman \& Monteith as simplified by the FAO,
  \item[--] the half-empirical model of Shuttleworth \& Wallace \citep{shuttleworth85}.
\end{itemize}
...
\begin{table}[ht]
  ...
  \begin{tabular*}{1.0\hsize}{llll}
    ...
    \hline
    Makk  & Makkink formula & crop factor    & soil moisture factor \\
    PM    & ---             & PM formula & \\
    FAO   & FAO PM formula  & crop factor    & soil moisture factor \\
    SW    & --- & &
  \end{tabular*}
\end{table}
...
The type of crop is regarded in terms of the Makkink crop factor (\verb!crop_makk!), which scales the $et$ flux compared to that of a \emph{reference crop} (well-watered grass of 0.08--0.13~m height, \citealt{feddes87}).
...
\textbf{Basics.}
The modified Penman-Monteith formula as published by the FAO (\url{http://www.fao.org/docrep/X0490E/x0490e00.htm}) gives a rough estimation of the potential evapotranspiration rate without the explicit use of resistances but empirically generalized factors.
...
A value of 1 corresponds to the reference crop (well-watered grass of 0.12~m height, 70~s~m$^{-1}$ \citep{unknown} or 69~s~m$^{-1}$ \citep{shuttleworth07} surface resistance, 0.23 albedo), values for other types of vegetation can be looked up online.
...
Although the reason is not yet clear to me, this makes it possible to add additional factors on the stomatal resistance through the preceding calculation of $r_{l,act}$, namely water stress and vapor pressure deficit.
The absence of other factors in the calculation procedure is discussed shortly in the documentation of ECHSE engines.
