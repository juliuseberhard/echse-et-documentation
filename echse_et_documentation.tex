...
\begin{denseitem}
  \item[--] \emph{Guiera senegalensis} (C3 shrub): 0.023373 hPa$^{-1}$,
  \item[--] \emph{Mitracarpus scaber} (C3 forb): 0.070084 hPa$^{-1}$,
  \item[--] \emph{Digitaria gayanus}\footnote{This name seems to be used almost only in this paper, not sure which plant they mean, maybe \emph{Andropogon gayanus}.} (C4 grass): 0.017004 hPa$^{-1}$,
  \item[--] \emph{Pennisetum glaucum} (C4 cereal grass): 0.013346 hPa$^{-1}$.
\end{denseitem}
%
...
\newpage
\subsection{Minimum stomatal resistance of single leafs (\texttt{res\_leaf\_min})} \label{ssec:parest_veg_resleafmin}

\textbf{Basics.}
The minimum stomatal resistance of single leafs $r_{l,min}$ is used in the calculation of the actual stomatal resistance $r_{l,act}$.
See Sec.~\ref{ssec:parest_veg_glohalf} for the details.

\noindent
\textbf{Method.}
Similarly to $g_{srad}$ (Sec.~\ref{ssec:parest_veg_glohalf}), \citet{saugier91} give values for some species (rather for conductance! ...),
\begin{denseitem}
  \item[--] alfalfa: 0.06~s~mm$^{-1}$ \citep{katerji83},
  \item[--] sunflower: xxxxx \citep{berger73},
  \item[--] Scots pine: xxxxxx \citep{lohammar80} and 150~W~m$^{-2}$ \citep{jarvis81},
  \item[--] apple tree: xxxxxxxx \citep{warrit80},
  \item[--] oil palm: xxxxx \citep{dufrene89},
\end{denseitem}
%
which I used for the estimation of $g_{srad}$ through model calibration (?).
